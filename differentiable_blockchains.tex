...
\begin{equation}
\Delta^q = \{x \in \mathbb{R} | \Sigma x_j = 1, x_j \geq 0 \forall j \}
\end{equation} 
...
\begin{equation}
P^\sigma_{*}(S): 0 \leftarrow \dots \leftarrow P^\sigma(S_{k-1})\xleftarrow {\partial_{k-1}} P^\sigma(S_{k})\xleftarrow {\partial_{k}} P^\sigma(S_{k+1}) \dots
\end{equation}
...
\end{equation}
for continuous functions $\delta^{i}_q: \Delta^{q-1} \rightarrow \Delta^q | 1 \leq i \leq q+1$ where % what is the "|" here?
\begin{equation}
...
\end{equation}
where $\Sigma_\pi: $ $ker \partial^{\pi}_{k}/im \partial^{\pi}_{k+1} \rightarrow ker \partial^{\pi+1}_{k} /im \partial^{\pi+1}_{k+1}$ 
